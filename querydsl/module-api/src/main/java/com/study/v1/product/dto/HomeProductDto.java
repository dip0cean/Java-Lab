...
public class HomeProductDto {
    ...
    public HomeProductDto(Product product) {
        ...
        this.salesPlatform = product.getSalesPlatform();

        // 옵션들 중 가장 낮은 가격
        this.webPrice = product.getLowestWebPrice();

        // 옵션들 중 가장 낮은 가격
        this.appPrice = product.getLowestAppPrice();

        // 정상가 / 웹가격  앱가격 중 가장 비싼 가격으로 책정
        this.tagPrice = product.getHighestPrice(webPrice, appPrice);
    ...
    public static Slice<HomeProductDto> entitiesToDtoList(
        ...
        List<HomeProductDto> products = sourceList
                ...
                .filter(dto -> isApp
                        ? dto.salesPlatform.isApp()
                        : dto.salesPlatform.isWeb())
                .toList();
